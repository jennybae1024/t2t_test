...
flags.DEFINE_string("file", "test", "global_steps.")
...
flags.DEFINE_string("t2t_usr_dir", "/home/ksk5693/t2t_systematic_generalization/usr_dir/",
...
def create_decode_hparams(shard):
    ...
    if FLAGS.global_steps:
        decode_hp.decode_to_file = str(FLAGS.global_steps)+FLAGS.file+str(shard)
    else:
        decode_hp.decode_to_file = FLAGS.file+str(shard)
...
def decode(estimator, hparams, decode_hp):
    ...
    if FLAGS.decode_interactive:
        ...
        decoding.decode_from_dataset(
            ...
            dataset_split=FLAGS.file if FLAGS.file == "test" else tf.estimator.ModeKeys.EVAL,
            checkpoint_path=FLAGS.checkpoint_path)
...
def main(_):
    ...
    # print(output_dir+"/model.ckpt-"+str(FLAGS.global_steps))
    # print(tf.train.latest_checkpoint(output_dir))
    # FLAGS.checkpoint_path = output_dir+"/model.ckpt-"+str(80000)
    if FLAGS.test_shard == -1:
        ...
        num = len([name for name in total_data_names if len(re.findall(FLAGS.file, name)) > 0])
        test_shards_list = [i for i in range(num)]
    elif FLAGS.test_shard == -2:
        test_shards_list = [0,3]
    elif FLAGS.test_shard == -3:
